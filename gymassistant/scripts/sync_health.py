...
Читаем КАЖДЫЙ РАЗ ВСЮ историю метрики — у API нет фильтра по дате. Это дёшево
(вся база здоровья — десятки тысяч документов, несколько мегабайт) и, что важнее,
самоисцеляюще: пропущенный по любой причине запуск не оставляет дыры, а
уточнённые задним числом замеры подхватываются сами. Апсерт по уникальному ключу
делает повтор бесплатным (database/orm_health.py).
...
# документов) отдаются мегабайтами, но у API нет ни пагинации, ни фильтра,
# так что запас нужен щедрый.
